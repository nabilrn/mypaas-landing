...
export const landingCopy = {
  en: {
    ...
    commandLabel: "Install or migrate", commandPlatform: "Ubuntu / Debian", copy: "Copy", copied: "Copied", copyFailed: "Copy failed",
    commandTabs: [
  ...
  id: {
    ...
    commandLabel: "Instalasi atau migrasi", commandPlatform: "Ubuntu / Debian", copy: "Salin", copied: "Tersalin", copyFailed: "Gagal menyalin",
    commandTabs: [
    ...
    faq: [["Apakah MyPaas merupakan hosted service?", "Bukan. Anda menginstalnya di VM Linux dan tetap bertanggung jawab atas host, domain, backup, dan kapasitas."], ["Apa yang dapat di-deploy?", "Repositori dengan Compose file di mana pun dalam tree, Dockerfile di root, atau output static yang memiliki index.html."], ["Apakah setiap Git push otomatis di-deploy?", "Bisa setelah webhook URL dan secret dari MyPaas ditambahkan ke GitHub. MyPaas belum mendaftarkan webhook secara otomatis."], ["Bagaimana project dipublikasikan?", "Caddy mengelola route internal. Cloudflare Tunnel menyediakan ingress publik dan TLS untuk hostname root serta wildcard project."], ["Bisakah MyPaas dipindah ke server lain?", "Bisa. Siapkan paket migrasi dari pengaturan dashboard, lalu jalankan perintah satu baris yang dihasilkan di VM baru. Perintah itu memulihkan database, volume, konfigurasi, dan secret terenkripsi, lalu men-deploy ulang project Anda."], ["Apa yang sengaja tidak didukung?", "Pendaftaran publik, billing, buildpack, Kubernetes, high availability multi-node, deployment multi-region, dan horizontal scaling otomatis."]],
  ...
  zh: {
    ...
    commandLabel: "安装或迁移", commandPlatform: "Ubuntu / Debian", copy: "复制", copied: "已复制", copyFailed: "复制失败",
    commandTabs: [
    ...
    operations: [["部署状态", "任务在后台运行时，排队、构建、运行、停止和失败状态始终可见。"], ["日志与指标", "构建日志、运行日志、CPU、内存、运行时间和 Compose 服务上下文都保留在项目中。"], ["环境变量", "部署前可检查检测到的变量名；保存的值会加密，并在普通界面中隐藏。"], ["生命周期", "部署、启动、停止、重启和重新部署都是明确的运维操作。"], ["历史与回滚", "发布记录保留提交和失败上下文，并可恢复到之前成功的发布。"], ["资源边界", "命名的 CPU 与内存配置将资源限制和实时使用情况分开显示。"], ["服务器迁移", "将整个平台——系统与项目数据库、数据卷、Compose 文件和加密密钥——导出为一个归档包，然后在新虚拟机上用一条命令完成恢复。"]],
    ...
    faq: [["MyPaas 是托管服务吗？", "不是。你需要将它安装到 Linux 虚拟机，并自行负责主机、域名、备份和容量。"], ["它可以部署什么？", "目录树任意位置包含 Compose 文件、根目录包含 Dockerfile，或包含 index.html 静态输出的仓库。"], ["每次 Git push 都会自动部署吗？", "将 MyPaas 生成的 webhook URL 和密钥添加到 GitHub 后可以自动部署；MyPaas 目前不会代你注册 webhook。"], ["项目如何公开访问？", "Caddy 管理内部路由，配置好的 Cloudflare Tunnel 为根域名和项目通配符域名提供公开入口与 TLS。"], ["可以把 MyPaas 迁移到另一台服务器吗？", "可以。在仪表盘设置中生成迁移包，然后在新虚拟机上运行生成的一行命令。它会恢复数据库、数据卷、配置和加密密钥，并重新部署你的项目。"], ["哪些功能明确不在范围内？", "公开注册、计费、buildpack、Kubernetes、多节点高可用、多区域部署和自动水平扩容。"]],
    statsLabel: "平台事实",
...
export function searchEntries(locale: Locale): SearchEntry[] {
  ...
  return [
    ...
    { title: locale === "zh" ? "迁移" : locale === "id" ? "Migrasi" : "Migration", section: docsSection, description: "Move a complete MyPaas installation to a new VM with the migration wizard.", href: "/docs/migration/", keywords: "migration migrate move vm server export import backup restore" },
